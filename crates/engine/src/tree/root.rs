...
pub fn calculate_state_root<BPF>(
    trie: &mut SparseStateTrie,
    state: HashedPostState,
    provider_factory: &BPF,
    ...
    BPF::StorageNodeProvider: TrieNodeProvider + Send + Sync,
{
    ...
    state
        ...
        .map(|(address, storage, storage_trie)| {
            ...
            let storage_provider = provider_factory.storage_node_provider(address);
                ...
                storage.storage.into_iter().sorted_unstable_by_key(|(hashed_slot, _)| *hashed_slot)
            {
                ...
                if value.is_zero() {
                    ...
                    storage_trie
                        .update_leaf(nibbles, alloy_rlp::encode_fixed_size(&value).to_vec(), &storage_provider)?;
                }
            }
    ...
    let (root, _updates) = trie.root_with_updates(provider_factory)?;
    Ok(root)
}
